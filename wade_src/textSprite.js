...
TextSprite.prototype.getScreenBoundingBox = Sprite.prototype.getScreenBoundingBox;
...
TextSprite.prototype._updateSize = function()
{
    ...
    if (!TextSprite.textMeasuring)
    {
        TextSprite.textMeasuring = {};
        TextSprite.textMeasuring.text = document.createElement('span');
        TextSprite.textMeasuring.text.style.whiteSpace = 'nowrap';
        TextSprite.textMeasuring.block = document.createElement('div');
        TextSprite.textMeasuring.block.style.display = 'inline-block';
        TextSprite.textMeasuring.block.style.width = '1px';
        TextSprite.textMeasuring.block.style.height = '0px';
        TextSprite.textMeasuring.div = document.createElement('div');
        TextSprite.textMeasuring.div.style.whiteSpace = 'nowrap';
        TextSprite.textMeasuring.div.style.position = 'absolute';
        TextSprite.textMeasuring.div.appendChild(TextSprite.textMeasuring.text);
        TextSprite.textMeasuring.div.appendChild(TextSprite.textMeasuring.block);
    }
    ...
    var textTop = TextSprite.textMeasuring.text.getBoundingClientRect().top;
    var height = TextSprite.textMeasuring.block.getBoundingClientRect().top - textTop;
    TextSprite.textMeasuring.block.style.verticalAlign = 'baseline';
    if (!height)
    {
        height = context.measureText('m').width * 3 + this._outlineWidth;
    }
    this._centerOffset.y = height / 2 + (textTop - TextSprite.textMeasuring.block.getBoundingClientRect().top);
...
TextSprite.prototype.draw_2d = function(context)
{
    if (context.isWebGl)
    {
    ...
    if (this._visible && this._text)
    {
        if (this._image)
        {
            ...
            Sprite.prototype.draw_2d.call(this, context);
...
TextSprite.prototype.draw_gl = function(context)
{
    if (!context.isWebGl)
    {
        TextSprite.prototype.draw_2d.call(this, context);
        return;
    }
    if (this._visible && this._text)
    {
        ...
        for (var i=0; i<separateLines.length && (!this._maxLines || currentLine < this._maxLines); i++)
        {
            ...
            while (words.length > 0 && idx <= words.length)
            {
                ...
                if (w > this._maxWidth && this._maxWidth > 0)
                {
                    ...
                    if (!this._maxLines || currentLine < this._maxLines)
                    {
                        if (draw)
                        {
            ...
            if (idx > 0 && (!this._maxLines || currentLine < this._maxLines))
            {
                text = words.join(' ');
                if (draw)
                {
